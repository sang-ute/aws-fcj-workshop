---
title: "Triển khai hàm Lambda: livenessResultFunction"
date: "`r Sys.Date()`"
weight: 8
chapter: false
pre: " <b> 7.3 </b> "
...
Trang này mô tả cách tạo hàm Lambda **`livenessResultFunction`**.  
Nó được sử dụng để truy xuất **kết quả Phiên Face Liveness** từ Amazon Rekognition, thực hiện so khớp khuôn mặt và lưu trữ dữ liệu trong Amazon DynamoDB/S3.
...
3. Trong phần **Author from scratch**:
   ...
   - **Function name:** `livenessResultFunction`
...
## Bước 2: Thêm mã cho hàm

1. Trong tab **Code**, thay thế mã mặc định bằng trình xử lý từ dự án Amplify của bạn:

![alt text](image-2.png)

Hoặc sao chép tệp `handler.js` từ:  
`Amplify/functions/livenessResultFunction/handler.js`
...
class RekognitionService {
  ...
  static async compareFaces(
    sourceImage,
    targetImage,
    threshold = CONSTANTS.FACE_MATCH_THRESHOLD
  ) {
  ...
  static async searchFacesByImage(
    collectionId,
    imageBuffer,
    threshold = CONSTANTS.FACE_MATCH_THRESHOLD,
    maxFaces = 5
    ...
    if (compareFaceId) {
      ...
      try {
        ...
        if (compareResult.FaceMatches && compareResult.FaceMatches.length > 0) {
          ...
          console.log(
            `🎉 Face match with ${compareFaceId}: ${bestMatch.Similarity.toFixed(
              1
            )}%`
          );
      ...
      if (searchResult.FaceMatches && searchResult.FaceMatches.length > 0) {
        ...
        console.log(
          `🎉 Best face match: ${name} with ${similarity.toFixed(
            1
          )}% similarity`
        );
...
```ini
S3_BUCKET=TÊN_S3_BUCKET_CỦA_BẠN
REKOGNITION_COLLECTION=TÊN_REKOGNITION_COLLECTION_CỦA_BẠN
DYNAMO_TABLE=TÊN_DYNAMO_TABLE_CỦA_BẠN
```
...
Hàm Lambda của bạn cần quyền để truy xuất kết quả Phiên Liveness, tìm kiếm khuôn mặt, phát hiện khuôn mặt, so sánh khuôn mặt, thêm dữ liệu vào DynamoDB và tương tác với S3 bucket của bạn.

1. Trong tab **Configuration**, đi tới **Permissions** và nhấp vào tên vai trò được gắn với hàm Lambda của bạn.
...
3. Chuyển sang chế độ xem **JSON** và dán nội dung:

{{% notice warning %}}
Thay thế ARN của S3 bucket và DynamoDB/collection bằng tên tài nguyên thực tế của bạn nếu khác. Cũng như AWS Region, AWS Rekognition collection, đừng chỉ dán JSON này, hãy đọc kỹ!!!
{{% /notice %}}
...
4. Lưu policy và gắn nó vào vai trò thực thi Lambda của bạn.

---

Tại thời điểm này, **`livenessResultFunction`** đã sẵn sàng để được tích hợp nhằm truy xuất kết quả liveness, chạy so khớp khuôn mặt và lưu trữ dữ liệu.
